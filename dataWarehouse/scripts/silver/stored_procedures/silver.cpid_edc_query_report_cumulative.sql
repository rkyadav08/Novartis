CREATE OR ALTER PROCEDURE silver.load_cpid_edc_query_report_cumulative
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        PRINT '================================================';
        PRINT 'Loading Silver Layer: CPID EDC Query Report Cumulative';
        PRINT '================================================';
        TRUNCATE TABLE silver.cpid_edc_query_report_cumulative;
        SET @start_time = GETDATE();

        INSERT INTO  silver.cpid_edc_query_report_cumulative (study_id, region, country, site_id, subject_id, folder_name, form_name, field_oid, log_no, visit_date, query_status, action_owner, marking_group_name, query_open_date, query_response, days_since_open, days_since_response, dwh_create_date)
        SELECT
            ...
            CASE WHEN query_response IS NULL OR LTRIM(RTRIM(query_response)) = '' THEN NULL WHEN query_response LIKE '%/%/%' THEN TRY_CONVERT(DATE, query_response, 101) WHEN query_response LIKE '%-%-%' AND LEN(LTRIM(RTRIM(query_response))) <= 10 THEN TRY_CONVERT(DATE, query_response, 10) ELSE TRY_CONVERT(DATE, query_response, 106) END AS query_response,
            ISNULL(days_since_open, 0) AS days_since_open,
            ISNULL(days_since_response, 0) AS days_since_response,
            GETDATE() AS dwh_create_date
        FROM bronze.cpid_edc_query_report_cumulative WHERE study_id IS NOT NULL;
